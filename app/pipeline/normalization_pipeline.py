...
from app.services.variable_metadata_builder import build_variable_metadata
...
def run_normalization_pipeline(
    ...
    # --- Stage 4c: Build variable metadata ----------------------------------
    metadata_result = build_variable_metadata(df, column_types)
    column_metadata = metadata_result["column_metadata"]

    # --- Stage 5: Build artifacts --------------------------------------------
    artifacts = build_artifacts(
        dataset, df, schemas, transformation_log, resolver_trace,
        column_types, column_metadata,
    )
